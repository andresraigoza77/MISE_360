...
def run_fix():
    ...
    print(f"Applying final fix v2 for {FILE}...")
    ...
    print(f"Found {len(ids_found)} Indicators.")
    print(f"Found {len(reportes_list)} report values to update.")
    ...
    for i in ids_found:
        c.execute("UPDATE Indicadores SET id_dependencia_responsable = ? WHERE id_indicador = ?", (DEP_ID, i))
